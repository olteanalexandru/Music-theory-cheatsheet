...
// Every duration expressed in sixteenth-note ticks, so measures can be filled with
// exact integer arithmetic instead of floating-point beat math.
...
export function generateRhythmPattern(timeSig: TimeSignatureName, difficulty: EarTrainingDifficulty): RhythmEvent[] {
    const totalTicks = TIME_SIGNATURES[timeSig].beatsPerMeasure * 4;
    ...
    while (remaining > 0) {
        const candidates = palette.filter((duration) => DURATION_TICKS[duration] <= remaining);
        ...
        events.push({ type: isRest ? 'rest' : 'note', duration, beats: ticks / 4 });
        remaining -= ticks;
    }
