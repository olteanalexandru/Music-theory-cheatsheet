...
2. **Run the schema migration**: open your project's *SQL Editor* and run the contents of [`supabase/schema.sql`](./supabase/schema.sql). This creates the `progress`, `curriculum_progress`, `review_progress`, `gamification`, `uploaded_files`, `profiles`, `follows`, `activity_events`, `activity_comments`, `activity_reactions`, `challenges`, `notifications`, `support_tickets`, and `support_ticket_messages` tables (with row-level security so each user can only see their own data, except `profiles` rows marked public and their related rows — `gamification`, activity feed events, etc. — which are readable by anyone for the leaderboard/profile/feed pages, and support tickets, which admins can also read/manage) and a private `play-along-files` storage bucket for saved Guitar Pro / MIDI files.
...
### Granting admin access (Support inbox)

The Support feature (`/support`) lets any signed-in user open a ticket. Replying to tickets and changing their status from the admin inbox at `/admin/tickets` requires the `is_admin` flag on a user's `profiles` row, which is `false` by default for everyone and has no in-app UI to grant it. To make an account an admin, run this once in the SQL Editor (replacing the email):
