...
2. **Run the schema migration**: open your project's *SQL Editor* and run the contents of [`supabase/schema.sql`](./supabase/schema.sql). This creates the `progress` and `uploaded_files` tables (with row-level security so each user can only see their own data) and a private `play-along-files` storage bucket for saved Guitar Pro / MIDI files.
